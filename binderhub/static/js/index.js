...
function getBuildFormValues() {
  var providerPrefix = $("#provider_prefix")
    .val()
    .trim();
  var repo = $("#repository")
    .val()
...
function build(providerSpec, log, path, pathType) {
  ...
  if ($("div#loader-text").length > 0) {
    ...
    window.setTimeout(function() {
      $("div#loader-text p").html(
        "Repository " +
          spec +
          " is taking longer than usual to load, hang tight!"
      );
    }, 120000);
  }
  ...
  image.onStateChange("failed", function(oldState, newState, data) {
    ...
    $("div#loader-text p").html(
      "Repository " +
        spec +
        " has failed to load!<br />See the logs for details."
    );
...
function indexMain() {
  ...
  $("#repository").on("keyup paste change", function(event) {
    updateUrls();
  });

  $("#ref").on("keyup paste change", function(event) {
    updateUrls();
  });

  $("#filepath").on("keyup paste change", function(event) {
    updateUrls();
  });
